...
Thus, the impact of mutational processes can be inferred by considering the DNA context of a polymorphism \citep{Alexandrov2013,Shiraishi2015a}. 
...
To build a test statistic to represent the association across all populations simultaneously, we performed a simple logistic regression predicting genotype based on $Q$. 
To account for population structure, we also included the top five global principal components.
This method identifies a total of 14,322 statistically significant variants associated to $Q$ (Figures \ref{NRS_Manhattan},\ref{NRI_Manhattan}, \ref{RS_Manhattan}, and \ref{RI_Manhattan}). 
...
From the list of 14,322 variants identified as being associated to $Q$, only 17 variants were present in HapMap \citep{HapMap2005}.
...
After downsampling the 1kGP data to match the resequenced individuals, we find that 9 out of the 296 the suspicious variants from the single population test are present in the resequenced data \ref{90HanSFS}.
...
Of the 14,322 suspicious variants identified globally, only 8,697 of them are found in the 83 individuals in the original 1kGP data.
From this subset, 8,085 of these variants are missing entirely from the resequenced data.
We noticed a unusually large number of INDELs that are fixed in the resequenced data that are at lower frequencies in the 1kGP. (See Figure \ref{90HanSFS_full})
...
\caption{Site frequency spectrum plot comparing the original 1kGP data to the high depth resequence data. This plot shows  8697 significant SNPs and INDELs identified in the \textit{5PC} model including all populations. Only 612 of the variants are present in the high depth resequenced individuals.}  
\label{90HanSFS_full}
\end{figure}
