...
These mismatches were present despite filtering for low-quality regions of the human genome and Hardy-Weinberg disequilibrium. \sgcomment{Despite considering only sites that satisfied strict quality masks and Hardy-Weinberg equilibrium in both cohorts? } 
...
These models were broadly consistent (See Figure \ref{CompareModel}). \sgcomment{Is this not redundant with the paragraph before last?}

\subsection{Cell line or technical artifact}

In 2017, Lan et al. resequenced 83 Han Chinese individuals from the 1kGP \citep{Lan2017}. Here we consider whether variants that, in the 1kGP, are both associated with $Q$ and polymorphic in the 83 individuals that were resequenced.

Among the 296 variants associated with $Q$ within the 1kGP CHB and CHS, and polymorphic the 83 resequenced individuals, only 9 are present in the resequenced data (Figure \ref{90HanSFS}). This is more than our nominal false positive rate of 1\% of the sites. Thus a small number of variants associated to $Q$ are present in the population but with somewhat biased genotypes. However, the vast majority of polymorphisms associated with $Q$ are not present at all in the resequencing dataset, supporting sequencing rather than cell line artifacts.
...
Among the 24,390 $Q$-associated variants, 613 are present on Illumina's Omni 2.5 chip \sgcomment{Figure}. These are likely among the small number of variants that are present in the data but exhibit biased genotyping in 1kGP. 
