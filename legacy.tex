...
To distinguish between weak but significant biases and dubious SNPS, we also considered the effect size and distinguished between variants where the allele frequency difference between low- and high-$Q$ individuals is larger than a factor of two.
We would expect the variants above this threshold to be enriched in real variants with genotyping biases.
We also note that the majority of the papers whose GWAS variants are associated to $Q$ are above this threshold (See Figure \ref{Pub_30}).
However, one study that has 7 variants below this threshold is a study that specifically looked at the copy number variation of EBV.
The results from this study could be confounded by the data quality of the 1kGP as these 7 variants are only significantly associated to EBV copy number in populations we identified as having strong batch effects.
