...
Reg <- fread(paste0(miscPath,'1KGP.GenomeWide.Regression_JPT.csv'), 
...
GWAS <- ggplot(NotSig, aes(x = Pos, 
              ...
              geom_point(alpha=0.3, 
                         size=1)+
              geom_point(data=SigPos, aes(x=Pos, 
                             y = plog10,),
                         color='black',
                         shape=3)+
              facet_grid(~Chr, 
...
if(!file.exists(paste0(miscPath,'jointAF.txt'))){
  ...
  f1kGP <- paste0(filterPath,
                  '1kGP_GenomeWide_JPT_1.frq')
  JPT <- fread(f1kGP, fill=T, 
               col.names=c('CHROM','POS','N_ALLELES',
                           'N_CHR','JPT_AF','JPT_MAF'))
  fnag <- paste0(filterPath,
                 'NAG_GenomeWide.4bed_filtered.freq.frq')
  NAG <- fread(fnag, fill=T,
               col.names=c('CHROM','POS','N_ALLELES',
