...
```{r}
#read Genotypes
han1=fread(paste0(DataPath,
                  'SignificantSNPs_Han90_0.01_genotype.txt'))
HanNames=fread(paste0(DataPath,
                      'SignificantSNPs_Han90_COLNAMES.txt'),
               header=F)
...
han <- han %>% 
  group_by(CHROM,POS,variable) %>% 
  summarize(han.gt = max(value))
#to get allele frequencies we group by POS and summarize GT
hanAF <- han %>% 
  group_by(CHROM,POS) %>% 
  summarize(hanAF = sum(han.gt))


#read Genotypes
kgp1 = fread(paste0(DataPath,
                    'SigVar_Pop_0.01_genotype.txt'))
ColNames = read.table(paste0(DataPath,
                             'ColNames.txt'),
                      header=F)
...
kgp <- kgp %>% 
  group_by(CHROM,POS,variable) %>% 
  summarize(Kgp.gt = max(value))
#to get allele frequencies we group by POS and summarize GT
kgpAF <- kgp %>% 
  group_by(CHROM,POS) %>% 
  summarize(kgpAF = sum(Kgp.gt))
...
##read Indels
Indels <- fread(paste0(DataPath,
                    'Reg_indels.POS'), 
                skip=1 ,col.names=c('CHROM','POS'))
Indels$Indel = 'yes'
...
for f in `seq 1 22`;
do echo $f;
bcftools view -R $DataPath/Significant_Positions.txt \
...
```{r}
#read Genotypes
GT = fread(paste0(DataPath,
                   'SigVar_Pop_0.01_rsID.genotype.txt'))
ColNames = read.table(paste0(DataPath,
                   'ColNames_rsID.txt'), 
                   header=F)
...
Sig.rsID <- fread(paste0(DataPath,
                         'SignificantVariants.txt'), 
...
over30 = mGT[which(mGT$avg_qual > 30),]

under30 = mGT[which(mGT$avg_qual < 30),]
