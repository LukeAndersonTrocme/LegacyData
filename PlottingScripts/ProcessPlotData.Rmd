...
#load SNP context
SNP <- read.table(paste0(DataPath,
                         'sig.pos.context.txt'), 
                sep = '\t', 
                header  = FALSE,
                col.names = contextHeader)
rSNP <- read.table(paste0(DataPath,
...
```{bash}
DataPath='/Users/luke/Documents/QualityPaper/ProcessedData'
...
##Full Model
```{r}
sharedID <- fread(paste0(DataPath,
                          '83samples.txt'), 
                  header = FALSE,
                  col.names = 'sample')

kgp_bim <- fread(paste0(DataPath,
                         'Significant_Positions.bim'), 
                  col.names = c('Chr','rsID','X',
                                'Pos','Ref','Alt')) %>%
  select(rsID)

kgp_fam <- fread(paste0(DataPath,
                         'Significant_Positions.fam'), 
                  col.names = c('familyID','sample',
                                'father','mother',
                                'sex','pheno')) %>%
  select(sample)

kgp_genotype <- read.bed(paste0(DataPath,'Significant_Positions'))
rownames(kgp_genotype) <- kgp_bim$rsID
colnames(kgp_genotype) <- kgp_fam$sample
#Only keep the 83 shared samples
keep <- colnames(kgp_genotype)[(colnames(kgp_genotype) 
                                 %in% sharedID$sample)]
kgp_genotype <- kgp_genotype[,keep]
kgp_genotype <- kgp_genotype[!duplicated(rownames(kgp_genotype)),]
kgp_genotype[1:5,1:5]

kgp1 <- melt(kgp_genotype, 
           varnames = c('rsID','sample'), 
           value.name = 'Genotype')
kgp <- 
		kgp1 %>% 
		group_by(rsID) %>% 
		summarize(kgp.AF = sum(Genotype))

kgp <- kgp[which(kgp$kgp.AF > 0),]
#read Han Genotypes
han_bim <- fread(paste0(DataPath,
                         'Significant_Positions_Han.bim'), 
                  col.names = c('Chr','rsID','X',
                                'Pos','Ref','Alt')) %>%
  select(rsID)

han_fam <- fread(paste0(DataPath,
                         'Significant_Positions_Han.fam'), 
                  col.names = c('familyID','sample',
                                'father','mother',
                                'sex','pheno')) %>%
  select(sample)

han_genotype <- read.bed(paste0(DataPath,'Significant_Positions_Han'))
rownames(han_genotype) <- han_bim$rsID
colnames(han_genotype) <- han_fam$sample
#Only keep the 83 shared samples
keep <- colnames(han_genotype)[(colnames(han_genotype) 
                                 %in% sharedID$sample)]
han_genotype <- han_genotype[,keep]
han_genotype <- han_genotype[!duplicated(rownames(han_genotype)),]
han_genotype[1:5,1:5]

han1 = melt(han_genotype, 
           varnames = c('rsID','sample'), 
           value.name = 'Genotype')
...
han <- 
		han1 %>% 
		group_by(rsID) %>% 
		summarize(han.AF = sum(Genotype))

#merge 1kGP and han resequenced data.
combo <- merge(han, kgp, by='rsID', all.y=T)
...
data_genotype <- read.bed(paste0(DataPath,'Significant_Positions'))
