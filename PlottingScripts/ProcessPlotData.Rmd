...
for(chrom in 1:22) {
  snp_list[[chrom]] <- 
    processContext(SNP %>% filter(Chr == chrom))
  if(max(snp_list[[chrom]]$Freq) > maxFreq){
    maxFreq <- max(snp_list[[chrom]]$Freq)
  }
  snp_list[[chrom]]$Chr <- chrom
  snp_combined <- rbind(snp_combined,snp_list[[chrom]])
}

plt_list <-list()
for(chrom in 1:22) {
  plt_list[[chrom]] <- 
    plotContext(snp_list[[chrom]],paste('Chrom',chrom)) + 
                theme(legend.position = "none")
}

plot_grid(plt_list[[1]] , plt_list[[2]], plt_list[[3]], plt_list[[4]], plt_list[[5]], plt_list[[6]], plt_list[[7]], plt_list[[8]], plt_list[[9]], plt_list[[10]], plt_list[[11]], plt_list[[12]], plt_list[[13]], plt_list[[14]], plt_list[[15]], plt_list[[16]], plt_list[[17]], plt_list[[18]], plt_list[[19]], plt_list[[20]],plt_list[[21]], plt_list[[22]], ncol=2)
...
snp_combined$title <- paste(snp_combined$Start,"*",snp_combined$End)

snp_combined <- merge(snp_combined, ChromLength, by = 'Chr')

snp_combined$norm_freq <- snp_combined$Freq / snp_combined$Length
...
ggplot(snp_combined, 
  ...
  theme(strip.placement = "outside",
        ...
        panel.border = element_rect(colour = "black", 
                                    ...
                                    size = 1))
ggsave(paste0(figurePath,
              'mutational_spectrum_per_chrom.jpg'),
              height=10, 
              width=12) 
