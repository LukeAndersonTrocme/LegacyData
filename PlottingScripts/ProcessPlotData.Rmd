...
jpt_imp_files = list.files(path=jpt_imp_path, 
                           pattern=paste0(array,".imputed.1.info.gz"),
                           full.names = T) 
# Read the files
...
imputed_position <- jpt_imp$Pos
suspicious_position <- jpt_imp[which(jpt_imp$Adjusted_Log10p > 2),]$Pos

jpt_imp$closest <- unlist(lapply(imputed_position, function(x) suspicious_position[which.min(abs(suspicious_position - x))]))
jpt_imp$dist <- abs(jpt_imp$closest - jpt_imp$Pos)

jpt_imp$dist_2 <- round(jpt_imp$dist, -1)
