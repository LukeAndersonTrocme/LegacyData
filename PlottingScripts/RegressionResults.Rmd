...
write.table(catalog[which(catalog$Adjusted_Log10p > -log10(0.01)),], 
            file = paste0(DataPath,
                          'SignificantPublications.txt'),
            row.names = FALSE, 
            quote = FALSE, 
            sep = ',')
...
ggplot(omni[which(omni$log10P_0.01 > 2),], 
       aes(x = ID, 
           y = log10P_0.01)) +
  geom_point(shape = 1) +
  labs(x = 'Allele Frequency',
       y = 'Adjusted -log10(p)') +
  ggtitle('219 SNPs present in the Omni 2.5 Microarray')
