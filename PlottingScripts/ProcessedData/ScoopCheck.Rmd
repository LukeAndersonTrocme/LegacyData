...
merged_regression_unmasked$strict_mask <- paste0("Pass strict mask (",nrow(merged_regression_unmasked),")")
merged_regression_masked$strict_mask <- paste0("Fail strict mask (",nrow(merged_regression_masked),")")
...
to_plot <- merged_regression %>%
           filter(pvalue_glm_filt < 1 | Adjusted_Log10p > 2)
...
p <- to_plot %>%
  filter(strict_mask == 'Pass strict mask')
...
#missing in Mafessoni
f <- to_plot %>%
  filter(strict_mask == 'Fail strict mask')
