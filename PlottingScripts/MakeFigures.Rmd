...
##PerMut PerChrom
```{r}
Reg_Context <- merge(Regression, rSNP, by=c('Chr','Pos'))
Reg_Context<-
  Reg_Context %>%
  select(Chr, Pos, rsID, 
         Adjusted_Log10p, 
...
Reg_Context$Rev<-ReverseComp(as.character(Reg_Context$Context))
Reg_Context$RevRef<-ReverseComp(as.character(Reg_Context$Ref))
Reg_Context$RevAlt<-ReverseComp(as.character(Reg_Context$Alt))
#only get one half (to fold over)
AC<-Reg_Context[which(Reg_Context$Ref 
                      %in% 
                        c('A','C')),]
TG<-Reg_Context[which(Reg_Context$Ref 
                      %in% 
                        c('T','G')),]
#fold over
TG$Context <- TG$Rev
...
data$Start <- substr(data$Mut,5,5)
data$End <- substr(data$Mut,7,7)
...
Reg_Context <- data %>% select(Chr, Pos, rsID, Adjusted_Log10p, title)

Reg_Context$Signature = 'other'
Reg_Context[grepl("AC → C",
                   Reg_Context$title),]$Signature <- '*AC→*CC'
Reg_Context[grepl(paste(signature,
                         collapse="|"),
                   Reg_Context$title),]$Signature <- 'T*T→TTT'

Reg_Context$Signature <- factor(Reg_Context$Signature, 
                                 levels = c("*AC→*CC", "T*T→TTT", "other"))

Reg_Context_Masked <- Reg_Context[which(Reg_Context$rsID %in% RegressionMasked$rsID),]
```
...
for(chrom in seq(1,22,1)){
  ...
  df_sig <- Reg_Context_Masked[which((Reg_Context_Masked$Chr == chrom) &
                           (Reg_Context_Masked$Adjusted_Log10p > -log10(0.01)) &
                             Reg_Context_Masked$Signature != 'other'),
                   c('Chr','Pos', 'Signature')]
  mask_sig <- Reg_Context[which((Reg_Context$Chr == chrom) &
                           (Reg_Context$Adjusted_Log10p > -log10(0.01)) &
                             !(Reg_Context$rsID %in% Reg_Context_Masked$rsID) &
                             Reg_Context$Signature != 'other'),
                   c('Chr','Pos', 'Signature')]
  ...
  if(chrom == 1){
    ...
    plts[[chrom]]<-ggplot(df_sig, aes(x=Pos)) +
    #geom_freqpoly(breaks = b, color = 'grey60') +
    #geom_freqpoly(data = df, breaks = b) +
      geom_freqpoly(data = df_sig, 
                    aes(color = Signature),
...
p1000_context <- plot_grid(plts[[1]],plts[[2]],plts[[3]],plts[[4]],plts[[5]],plts[[6]],plts[[7]],plts[[8]],plts[[9]],plts[[10]],plts[[11]],plts[[12]],plts[[13]],plts[[14]],plts[[15]],plts[[16]],plts[[17]],plts[[18]],plts[[19]],plts[[20]],plts[[21]],plts[[22]],nrow=1,rel_widths = relWidths)
...
return(p)
 ...
 for(i in seq(1,nrow(over10))){
   ...
   sub[[i]] <- Reg_Context_Masked[which((Reg_Context_Masked$Chr == chr) &
                              (Reg_Context_Masked$Pos > start) &
                              (Reg_Context_Masked$Pos < end)),]
   pp[[i]] <- makeSubPlot(sub[[i]],title)
 }

 plot_grid(pp[[1]], pp[[2]], pp[[3]], nrow =1)
  ggsave(paste0(figurePath,
                'RegionOver10.jpg'), 
       height=5, 
       width=10)
