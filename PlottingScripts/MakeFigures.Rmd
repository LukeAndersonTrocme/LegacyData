...
SFS <- 
  ...
  labs(x = "Nagahama Frequency",
       y = "1000 Genomes Project Frequency") +
  theme_classic() +
  theme(axis.line = element_blank(),
        axis.text = element_text(size = 20),
        axis.title = element_text(size = 20))

freq_gw <- as.data.frame(table(round(join$JPT_MAF, 2))) #as.data.frame(table(bin(join$JPT_MAF, 104)))
freq_gw <- freq_gw %>% filter(Var1 != "0") #(-0.001,0.00962]
freq_gw$proportion <- freq_gw$Freq / sum(freq_gw$Freq) * 100
freq_gw$SNPs <- "All"
freq_gw$Var1 <- as.numeric(as.character(freq_gw$Var1)) #as.numeric(as.character(substr(freq_gw$Var1, 2, 7)))

freq_sig <- as.data.frame(table(round(jj$JPT_MAF, 2))) # as.data.frame(table(bin(jj$JPT_MAF, 104)))
freq_sig$proportion <- freq_sig$Freq / sum(freq_sig$Freq) * 100
freq_sig$SNPs <- "Suspicious"
freq_sig$Var1 <- as.numeric(as.character(freq_sig$Var1))

freq <- rbind(freq_sig, freq_gw)
