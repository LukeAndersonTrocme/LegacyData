...
dummy <- 
  ggplot(SigPos, 
         aes(x = Pos, 
             y = deviance, 
             shape = SigPos$deviance > 40)) +
  geom_point() +
  scale_shape_manual(values = c(3,16),
                     labels = c('Yes', 'No')) +
  guides(shape = 
           guide_legend(title = 'Suspicious',
                        override.aes = 
                          list(size = 8,
                               color = c('blue',
                                         'grey70'))))+
...
gwas <- 
  ...
  scale_color_manual(values = rep(
    c("grey70", "grey30"), 
    22)) +
  labs(title = "Association of variant frequency to the average quality of mapped bases in 1kGP",
