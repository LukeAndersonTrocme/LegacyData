...
HIST <- 
  ...
  scale_y_reverse(expand = c(0.01, 0.01), 
                  breaks = c(10, 50, 100)) +
  labs(x = "1000 Genomes Project Frequency",
       y = "") +
  coord_flip() +
  theme_classic() +
  theme(axis.line = element_blank(),
...
SNPplt <- plotContext(SNP %>% filter(Mut == 'A-> C'),'Suspicious SNPs')
rSNPplt <- plotContext(rSNP %>% filter(Mut == 'A-> C'),'Control SNPs')
