...
freq_gw <- as.data.frame(table(round(join$JPT_MAF, 2)))
freq_gw <- freq_gw %>% filter(Var1 != "0")
freq_gw$proportion <- freq_gw$Freq / sum(freq_gw$Freq) *100
freq_gw$group <- "All SNPs"
freq_gw$Var1 <- as.numeric(as.character(freq_gw$Var1))

freq_sig <- as.data.frame(table(round(jj$JPT_MAF, 2)))
freq_sig$proportion <- freq_sig$Freq / sum(freq_sig$Freq) *100
freq_sig$group <- "Suspicious SNPs"
...
HIST <- 
  ggplot()+
  geom_histogram(data = freq_gw,
                 aes(x=Var1, 
                     y = proportion), 
                 fill = 'grey',
                 stat = "identity")+
  geom_histogram(data = freq_sig,
                 aes(x=Var1, 
                     y = proportion),
                 fill = 'blue',
                 alpha=0.7,
