...
GWAS <- 
  ...
  theme(plot.title = element_text(hjust = 0.5), 
        ...
        axis.title.x = element_text(size = 20,
                                  vjust = 7),
        plot.margin = unit(c(1.5,5,0,1.5), "lines"))
```
...
metaData$Population_tab <- sub("^","                                       ",
                           metaData$Population)
metaData$Population <- factor(metaData$Population_tab, levels = unique(metaData$Population_tab))
#metaData$Population <- factor(metaData$Population, levels = unique(metaData$Population))

a <- ggplot(metaData %>% arrange(x), 
            aes(x = sample,
                y = avg_qual,
                color = Population,
                shape = as.factor(Phase))) +
  geom_point(size=1) +
  facet_wrap(.~Population,  scales = 'free_x', nrow=1, switch = 'y') + #, space = 'free_x'
  geom_hline(yintercept = 30, 
             color = 'grey') +
  scale_colour_manual(values = MyColour_tab, 
                      breaks = unique(metaData$Population)) +
  scale_x_discrete(expand = c(0.5,0.5)) +
  scale_shape_manual(values = c(1,3),
                     name = 'Phase') +
  guides(color = FALSE) +
  labs(title = "Populations ranked by sequencing date",
...
a_t <- ggplot(metaData %>% arrange(x), 
            aes(y = sample,
                x = avg_qual,
                color = Population,
                shape = as.factor(Phase))) +
  geom_point() +
  facet_wrap(.~Population,  scales = 'free_y', ncol=1, switch = 'x') + #, space = 'free_x'
  ...
  labs(title = "Data quality over time",
       y = "Populations ranked by sequencing date",
       x = "Average quality of mapped bases") +
  theme(plot.title = element_text(hjust = 0.5), 
        legend.position = c(0.9, 0.95),
        axis.text.y = element_blank(), 
