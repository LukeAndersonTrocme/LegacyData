...
ggplot(plot, aes(x=1-AF.x, y=AF.y))+geom_point()+geom_point(data=plot[which(plot$rsID=='rs6057648'),], color='blue')+labs(x='Allele Frequency in HapMap', y='Allele Frequency in 1kGP', title='Significant SNPs found in HapMap and 1kGP')+theme_classic()
...
ggplot(plt, aes(y=AC1, x=AC_raw))+geom_point(shape=1)+labs(x='Allele Frequency in GnomAD', y='Allele Frequency in 1kGP', title='Significant SNPs found in GnomAD and 1kGP')+theme_classic()+xlim(c(0,1000))+ylim(c(0,1000))