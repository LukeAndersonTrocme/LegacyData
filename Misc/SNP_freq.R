...
pops<-list.files(path="~/Documents/QualityPaper",pattern=".frq", full.names=T)
...
pop$Pop=substr(pops[f],62,64) #get pop name from file
Freq<-rbind(Freq, pop)
}
...
ggplot(Freq, aes(x=ordered, y= as.numeric(Y.ALLELE.FREQ), color=ordered, group=1))+
geom_hline(yintercept=0, color='grey70', linetype=1)+
geom_hline(yintercept=0.5, color='grey70', linetype=3)+
geom_line()+
geom_point(shape=15)+
...
theme(strip.background=element_blank(), 
		...
		axis.text.x=element_text(angle=90, size=5, vjust=-0.05))

#+geom_point(data=plt[complete.cases(plt),], shape=1, size=4, color='black')
