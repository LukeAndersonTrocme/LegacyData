...
def main(args):
    ...
    Chunk = pd.read_table(args.i, header=None, 
                     names=Qual.Name, chunksize=10000, compression = 'gzip')
    count=1
    for GT in Chunk:
        GT = GT.reset_index().rename(columns = {'level_0':'Chr','level_1':'Pos'})
        print('Chunk ' + str(count) + ' is read')
        count=count+1
        ...
        for p in Qual.Pop.unique():
            ...
            if not os.path.isfile(fileName):
                GT_Qual.to_csv(fileName, mode = 'w', compression = 'gzip', index=False)
            else: # else it exists so append without writing the header
                GT_Qual.to_csv(fileName, mode = 'a', compression = 'gzip', header=False, index=False)
        #print('[DONE]')
