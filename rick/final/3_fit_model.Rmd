---
title: "Quality Genotype Association"
subtitle: "Fitting model for Chromosome `r commandArgs(trailingOnly=T)[1]`"
author: 
- name: Rick Farouni, 
  affiliation:
...
# Rscript -e 'library(rmarkdown); rmarkdown::render("test.Rmd", "all")'  "22_10000" 
...
bim_filepath <- file.path(plink_data_dir, paste0(file_name ,'.bim'))
fam_filepath <- file.path(plink_data_dir, paste0(file_name , '.fam'))
bas_filepath <- file.path(metadata_dir, "average_quality_fam.txt")
bed_filepath <- file.path(plink_data_dir, file_name )
lf_filepath <-   file.path(preprocessed_dir, "lf5.rds")  
...
# tic("load metadata.")
# fam_file <-read_delim(fam_filepath, 
#                       delim=" ", 
#                       col_names=c('sample','a','b','c','d','e')) %>%
#   select("sample")
#                  
# 
# bas_dt <- left_join(fam_file, 
#                 bas_dt,
#                 by = 'sample') 
# 
# basfam_filepath <- "./data/metadata/average_quality_fam.txt"
# write_tsv(bas_dt, basfam_filepath)
# toc()
...
```{r}
data_genotype %>% arrange(-dev_logp) %>% slice(1:10)
```
...
```{r}
tic("saving fit")
saveRDS(data_genotype, fits_filepath)
toc()
```
