...
```{r}
plink_data_dir <- "./data/plink/by_chrom"
metadata_dir <- "./data/metadata"
preprocessed_dir <- "./data/preprocessed"
bim_filepath <- file.path(plink_data_dir, paste0('chr', chr,'.bim'))
fam_filepath <- file.path(plink_data_dir, paste0('chr', chr, '.fam'))
bas_filepath <- file.path(metadata_dir, "average_quality_dt.txt")
bed_filepath <- file.path(plink_data_dir, paste0('chr', chr))
...
```{r}
data_fit %>% arrange(-dev_logp) %>% slice(1:10)
```
...
```{r}
tic("saving fit")
saveRDS(data_fit, fits_filepath)
toc()
```
