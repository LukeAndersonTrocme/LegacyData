...
#modified from gcat
fit_glm_dich <- function(..., LF, trait){
  snp <- c(...)
  snp <- as.numeric((snp==1) | (snp==2))
  ...
  b0 <- lreg_dich(snp_no_na, LF_no_na) #coefficients from logreg 
  b1 <- lreg_dich(snp_no_na, cbind(LF_no_na, trait_no_na))
  ...
  stat <-
    dataset %>%
    summarize( #cross entropy (log likelihood)
...
saveRDS(data_fit_dich, 
	file.path("./data/preprocessed/subsets",
	paste0("dich_fits_chrom_", chr, ".rds")))

write.table(data_fit_dich[-2], 
              file.path("./data/preprocessed/subsets", 
                        paste0("dich_fits_chrom_", chr, ".txt")),
              quote = FALSE,
              row.names = FALSE)
```
#PCA
...
write.table(data_fit_PCA[-2], 
              file.path("./data/preprocessed/subsets", 
                        paste0("fitsPCA_chrom_", chr, ".txt")),
              quote = FALSE,
              row.names = FALSE)

saveRDS(data_fit_PCA_dich, 
	file.path("./data/preprocessed/subsets",
	paste0("dich_fits_chrom_", chr, ".rds")))

write.table(data_fit_PCA_dich[-2], 
              file.path("./data/preprocessed/subsets", 
                        paste0("dich_fitsPCA_chrom_", chr, ".txt")),
              quote = FALSE,
              row.names = FALSE)
```
#Population
...
write.table(data_fit[-2], 
              file.path("./data/preprocessed/subsets", 
                        paste0("fitsPOP_chrom_", chr, ".txt")),
              quote = FALSE,
              row.names = FALSE)

saveRDS(data_fit_Pop_dich, 
	file.path("./data/preprocessed/subsets",
	paste0("dich_fits_chrom_", chr, ".rds")))

write.table(data_fit_dich[-2], 
              file.path("./data/preprocessed/subsets", 
                        paste0("dich_fitsPOP_chrom_", chr, ".txt")),
              quote = FALSE,
              row.names = FALSE)
```
...
plt <- merge(data_fit[-2], data_fit_PCA[-2], by=c('rsID'), suffixes = c('LFA','PCA'))
plt <- merge(plt, data_fit_Pop[-2], by=c('rsID'), suffixes = c('','Pop'))
...
#plot diff
plt <- merge(data_fit, data_fit_Pop_dich, by=c('rsID'), suffixes = c('LFA_Full','Pop_Dich'))
