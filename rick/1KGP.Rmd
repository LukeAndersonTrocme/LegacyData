...
chr20 <- sparseMatrix(i=x[,2], j=x[,1], x=1L)
...
```{r}
chr20_mtx <- t(as.matrix(chr20[,1:100000]))
```
...
fit_model_1 <- function(data, qual){
...
data_list <- 
  data_list %>%
  mutate(
    fit = future_map(dataset,~ glm(genotype ~ avg_qual_mean + offset(iaf), family ="binomial", data=.x)),
