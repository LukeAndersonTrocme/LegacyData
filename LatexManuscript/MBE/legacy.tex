...
\todo{This signature had been partially reproduced at continental resolution in the SGDP. 
However, due to smaller sample sizes, statistical power lacked to confirm the signature.
Moreover, the result had yet to be reproduced in a Japanese dataset, so we decided to follow up on this observation, by using a newly sequenced dataset of Japanese individuals from Nagahama \citep{Harris2017a,Aikens2018}.}
...
\todo{Chromosome 1 shows enrichments in the  *AC${\rightarrow}$*CC signal compared to other chromosomes despite accounting for chromosomal length (Figure \ref{MutEnrichment_chrom}).} 
...
\begin{figure}[tbp]
\centering
\includegraphics[width=10cm,keepaspectratio]{mutational_enrichment_per_chrom.jpg}
\caption{Mutation spectrum enrichment of  $Q$-associated variants across each chromosome.
Chromosome 1 has a strong enrichment in *AC${\rightarrow}$*CC mutational pattern.}  
\label{MutEnrichment_chrom}
\end{figure}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=11cm,keepaspectratio]{CountPerChrom.jpg}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=14cm,keepaspectratio]{VariantDensity.jpg}
