...
\todo{
An excess of this signature has been observed in East Asians compared to Afriacas in the SGDP. 
At a continental resolution, this signature has been partially reproduced in the SGDP -- an independent dataset with smaller sample size from the 1kGP \citep{Aikens2018, Harris2017a}. 
Nevertheless, the heterogeneous signature reported by \cite{Harris2017a} had yet to be reproduced in a Japanese dataset, so we decided to follow up on this observation, by using a newly sequenced dataset of Japanese individuals from Nagahama.}
The herertogeneous signatrue reported by harris et al had not , so we decided.
...
\todo{Chromosome 1 shows strong enrichments in the  *AC${\rightarrow}$*CC signal compared to other chromosomes (Figure \ref{CountPerChrom}B).} 
