...
\todo{While this signature had been partially reproduced in the SGDP, it had yet to be reproduced in a Japanese dataset, we decided to follow up on this observation, by using a newly sequenced dataset of Japanese individuals from Nagahama \citep{Harris2017a,Aikens2018}.}
...
The size of blue dots are proportional to the number of suspicious SNPs in a given frequency bin (out of the 1034 identified by GWAS).  
\textbf{C} 
Mutation spectrum of these 1034 variants compared to the expectation from the distribution of all SNPs.
...
Our initial goal was to determine whether this signature could be explained by population structure or by technological bias.
...
When comparing the allele frequencies between the Japanese individuals from the 1kGP and this larger dataset, we observed a number of single nucleotide polymorphisms (SNPs) private to one of the two groups (Figure \ref{SFS}B).
...
Statistical significance of these enrichments is computed using a chi-squared test following \cite{Harris2017a}.
...
For other analyses where quality of imputation matters, identifying and discarding $Q$-associated variants may be preferable. 
...
We therefore performed the same association approach in all populations independently, and similarly identified $Q$-associated SNPs in 23 of the 26 populations in the 1kGP, with the phase 1 populations being most affected, with on average four times as many significantly associated sites compared to the phase 3 populations. Over 812 variants were independently associated to low $Q$ in at least two populations with $ p < 10^{-6}$ in each.



To build a test statistic to represent the association across all populations simultaneously, we performed a simple logistic regression predicting genotype based on $Q$ with the logistic factor analysis (LFA) as an offset to account for population structure. This approach is called Genotype-Conditional Association Test  (GCAT) as proposed by \citep{song2015testing}. 
...
The GCAT method identifies a total of 24,390 variants associated to $Q$ distributed across the genome with 15,270 passing the 1kGP strict mask filter (Figures \ref{NRS_Manhattan},\ref{NRI_Manhattan}, \ref{RS_Manhattan}, and \ref{RI_Manhattan}). Most analyses below focus on the 15,270 variants satisfying the strict mask, since these variants are unlikely to be filtered by standard pipelines.
...
Interestingly, the variants identified by Mafessoni et al. are not enriched in the mutational spectra described above (Figure \ref{Mafessoni_MutSpect}B).
These results indicate that there may be a multitude of batch effects in the 1kGP that can only be identified using a suite of association tests.
...
{SNP}  & 3,369 &\textcolor{gray}{  $5.3\times 10^{-4}$}& 11,059 &\textcolor{gray}{  $5.6\times 10^{-4}$}& 14,428\\  
{INDEL} & 181& \textcolor{gray}{  $3.0\times 10^{-4}$}& 657& \textcolor{gray}{  $6.6\times 10^{-4}$} & 838\\ \hline
...
Because this is higher than the false discovery rate, we conclude that the cohort-wide GCAT test identifies predominantly spurious variants, but also variants that have biased genotypes. The similarity in allele frequencies for these variants suggests that the biases in allele frequencies are present in populations other than the CHB and CHD. 
...
This suggests that the imputation reference panel still includes individuals with low $Q$, and the dubious variants will be imputed in individuals who most closely match the individuals with low $Q$.
These $Q$-associated variants could also compromise the imputation of nearby real variants because haplotype matching would be compromised. However when considering the imputation scores of genotype data from Japanese individuals from Nagahama, there does not appear to be any impact on nearby variant imputation scores (Figure \ref{imputation_score}).

We searched the literature for GWAS that have reported these $Q$-associated variants as being significantly associated with some biological trait, even though there is no particular reason for these variants to be associated with phenotypes.
...
Despite state-of-the-art quality controls, the variants were imputed onto genotype data, reached genome wide significance for association with biological traits, and were reported to the GWAS catalogue.
Removing these variants from imputation servers could help avoid spurious associations.

These associations are not necessarily incorrect -- a weak but significant bias in imputation may still result in a correct association.
...
Even though these seven variants are reported in the GWAS catalogue, they were not flanked by LD peaks and were correctly removed from downstream analyses by \citep{Mandage2017}. 
It is plausible that the EBV copy number phenotype estimation is sensitive to mapping and confounded by the same technical artefacts that lead to biased SNP calling.
...
Given the progress in sequencing and calling that occurred since the early phases of the 1kGP (Figure \ref{MapQual}), it is likely that the source of these biases is not longer actively introducing bias in recent sequence data.
...
On a technical front, we were surprised that such strong associations between variants and technical covariates in the 1kGP project had not been identified before. 
...
Since this analysis is primarily performed using publicly available data, we provide fully reproducible and publicly available pipelines on \href{https://github.com/LukeAndersonTrocme/LegacyData}{GitHub}.
...
For the mutation spectrum analysis, we reproduced the quality control and data filtering pipelines used by Harris et al. 
...
For the GCAT analysis, the only filtration used was the application of an minor and major allele frequency cutoff of 0.000599 (removing singletons, doubletons and tripletons) resulting in a total of S=28,516,063 variants included in the test. 
...
Variants flagged by the 1kGP strict mask are included in the association test and included in the four separate FDR adjustments for the four categories of variants (INDELs in Repeat and Non-Repeat regions and SNPs in Repeat and Non-Repeat regions). 
...
The deviance test statistic under the null model is approximately chi-square distributed with one degree of freedom.
...
By using a nominal Type-I error rate $\alpha = 0.01$, a total of 15,270 variants were found to be statistically significant. 
...
These methods model each individual-specific allele frequency as a logistic function using principal components (PC), Population membership (Pop) or LFA as an offset.
...
\caption{ \textbf{A} Comparison of the p-values from the LD-based association from \cite{mafessoni2018turning} to p-values from the $Q$ association performed here.
...
For the 14,616 variants that do not pass the strict mask, 650 ( 3\% ) variants are both $Q$-associated and LD-associated; 14,616 variants ( 70\% ) are $Q$-associated but not LD-associated and 5,625 ( 27\% ) are LD-associated but not $Q$-associated.
...
\caption{Manhattan plot of the $-\log_{10}(p)$ values for the GCAT logistic regression analysis for the 10kb window with the most  $Q$-associated variants per 10kb across the genome.}  
\label{Top10}
\end{figure}
...
\caption{Site frequency spectrum plot comparing the allele frequency difference between individuals with low- and high-$Q$. The black dashed lines indicates equal allele frequencies while the red dotted line indicates variants twice as frequent in individuals with $Q$ scores below 30. The majority (92.7\%) of the $Q$-associated variants are more than twice as frequent in individuals with low-$Q$.}  
\label{90HanSFS_full}
\end{figure}
...
\caption{Site frequency spectrum plot comparing the frequency of $Q$-associated variants identified in GWAS publications, for individuals with $Q$ scores above and below 30. The black dashed lines indicates equal allele frequencies while the red dotted line for variants twice as frequent in individuals with $Q$ scores below 30. Each of the rsIDs of the variants are labelled for clarity.}  
\label{Pub_30}
\end{figure}
...
\caption{Joint site frequency spectrum plot of 1000 Genomes Project data and the high depth resequence data, restricted to variants that are both associated with $Q$ in single population tests and polymorphic in the 83 individuals that were resequenced.
Among the 296 variants associated with $Q$ in the single population tests within the 1000 Genomes Project CHB and CHS, 6 are present in the resequence data \citep{Lan2017}.}  
\label{90HanSFS}
\end{figure}
...
\caption{Joint frequency spectrum for the original 1000 Genomes Project data and the high depth resequence data for variants that, in the 1000 Genomes Project, are both associated with $Q$ and polymorphic in the 83 individuals that were resequenced.
...
\begin{figure} \centering
    ...
    \textbf{A} Manhattan plot of the $-\log_{10}(p)$ values for the GCAT logistic regression analysis for SNPs in non repetitive regions. 
...
\begin{figure} \centering
    ...
    \end{subfigure} 
    \caption{Association of indels in non-repetitive regions with $Q$. \textbf{A} Manhattan plot of the $-\log_{10}(p)$ values for the GCAT logistic regression analysis for INDELs in non repetitive regions. 
    ...
    The circles ( o ) are variants that reached values greater than 20, for clarity we set a hard ceiling at 20. 
...
\begin{figure} \centering
    ...
    \end{subfigure} 
    \caption{Association of SNPS in repetitive regions with $Q$. \textbf{A} Manhattan plot of the $-\log_{10}(p)$ values for the GCAT logistic regression analysis for SNPs in repetitive regions. 
    ...
    The circles ( o ) are variants that reached values greater than 20, for clarity we implemented hard ceiling at 20. 
  \textbf{B} QQ plot of the unadjusted p values for the GCAT logistic regression analysis for SNPs in repetitive regions.}
  \label{RS_Manhattan}
  \end{figure}
\begin{figure} \centering
    \begin{subfigure}[b]{\linewidth}
    ...
    \end{subfigure} 
    \caption{Association of indels in repetitive regions with $Q$.  \textbf{A} Manhattan plot of the $-\log_{10}(p)$ values for the GCAT logistic regression analysis for INDELs in repetitive regions. 
    ...
    The circles ( o ) are variants that reached values greater than 20, for clarity we implemented hard ceiling at 20. 
  \textbf{B} QQ plot of the unadjusted p values for the GCAT logistic regression analysis for INDELs in repetitive regions.}
  \label{RI_Manhattan}
  \end{figure}
