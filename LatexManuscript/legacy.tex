...
\todo{Mafessoni et al. recently identified a batch effect in the 1kGP without using any quality control metrics, but instead, using an approach that identifies individuals and loci with unusual linkage disequilibrium (LD) patterns, these result partially overlap with the variants we identified using a more statistically powerful method \citep{mafessoni2018turning}.}\sgcomment{You are talking about things that have not been introduced yet... Maybe: Mafessoni et al. recently identified batch effects in the 1kGP by looking for individuals with excess LD among distant variants. 
}
...
\caption{
Suspicious mutations carried by individuals with low quality data have distinct mutational profiles, reproduce poorly across studies \sgcomment{, and are distributed across the genome--this is outdated}. \luke{not sure what this means?}
\textbf{A} 
...
\todo{Our initial goal was to determine if \sgcomment{whether?} this signature could be explained by population structure or technological error.}
...
\todo{A recent publication by Mafessoni et al. also identified a batch effect in the 1kGP using a method that is independent of quality metrics.
They identify over 15,000 variants significantly associated to abnormal LD patterns that are not associated to $Q$ \ref{Mafessoni_MutSpect}.
...
Interetingly, the variants identified my Mafessoni et al. are not enriched in the mutational spectra described above \ref{Mafessoni_MutSpect}.
...
\todo{
To assess whether or not $Q$-associated variants results from mutations occurring in the creation or maintenance of the cell lines, or from technical artifacts, we compare the original 1kGP sequence data to more recent sequencing efforts using the same cell lines.
Variants that do not reproduce across these cohorts are likely the result of technical artifacts. By contrast, if the same mutations are observed at identical frequencies in independently sequenced cohorts, we can conclude that recurrent cell line mutations are likely to blame. Finally, to complicate matters, we may have a third situation where a real variant has a genotyping error probability that correlates with $Q$. In this case, we expect the variant may reproduce, but at different frequency.
 }
