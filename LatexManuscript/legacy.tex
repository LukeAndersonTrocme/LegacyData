...
\maketitle
\begin{abstract}
Recent reports have identified differences in the mutational spectra across human populations. While some of these reports have been replicated in other cohorts, most have been reported only in the 1000 Genomes Project (1kGP) data. While investigating an intriguing putative population stratification within the Japanese population, we identified a previously unreported batch effect leading to spurious mutation calls in the 1kGP data and to the apparent population stratification. Because the 1kGP data is used extensively, we find that the batch effects also lead to incorrect imputation by leading imputation servers and \luke{some questionable \sout{suspicious}} GWAS associations. Lower-quality data from the early phases of the 1kGP thus continues to \luke{impact \sout{contaminate}} modern studies in hidden ways. \luke{As technologies and analytical methods improve, it is important that we renew and update these data resources accordingly}\sout{It may be time to retire or upgrade such legacy sequencing data.}
\vspace{5mm}
...
\luke{Given this signature had not been reproduced in another dataset}, we \sout{therefore} decided to follow up on this observation, by using a newly sequenced dataset of Japanese individuals from Nagahama.
\luke{Our initial goal was to explore the population structure that might be responsible for such an intriguing mutational signature.}
...
Stars ( * ) indicate significant difference from the mutational spectrum of a set of control SNPs.
}
 \label{SFS}
\end{figure}
...
\luke{\textbf{B} The sequencing technologies used over time. A change in technology coincides with an increase in overall quality of data produced.}}
\label{MapQual}
\end{figure}
...
Here we perform a \luke{genotype conditional association test (GCAT), where} genotypes are now the dependent variable that we attempt to predict using the continuous variable $Q$ as the independent variable \citep{song2015testing}.
...
\luke{Statistical significance of these enrichment is computed using a chi-squared test as described by Harris 2017 \citep{Harris2017a}. }

Thus, this mutational signal is heavily enriched in $Q$-associated SNPs, but residual signal remains in non-significant SNPs, presumably because many rare alleles found in individuals with low $Q$ remain unidentifiable using association techniques \luke{due to lack of power (Supplementary Figure 000}).
...
This is more than our nominal false discovery rate of \luke{$\alpha = 0.01$} of the sites. 
...
\luke{In the GCAT test, a variant may be significantly associated to $Q$ in some populations, but not necessarily all populations. 
Thus, the 1,139 variants present in the resequenced Han might have stronger associations to $Q$ in in individuals from other populations.}
There are 5 \luke{variants} that show differing frequencies between both datasets that are likely explained by biased genotypes.
...
\luke{These  $Q$-associated variants could also compromise the imputation of real variants that happen to be  close by, causing not only improper imputation, but also false negatives for real variants.}
...
\luke{Some of these publications noted that these variants were not flanked by linkage disequilibrium peaks \citep{Mandage2017}.
...
\luke{It is unlikely that EBV copy number associations to these seven $Q$-associated variants could be explained by population structure as the data quality of an individual is also correlated with sequencing date.
This suggests that there may be issues with how EBV is mapped to the genomes of individuals with low quality sequence data.}
