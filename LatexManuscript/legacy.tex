...
\maketitle
\begin{abstract}
Recent reports have identified differences in the mutational spectra across human populations. While some of these reports have been replicated in other cohorts, most have been reported only in the 1000 Genomes Project (1kGP) data. While investigating an intriguing putative population stratification within the Japanese population, we identified a previously unreported batch effect leading to spurious mutation calls in the 1kGP data and to the apparent population stratification. Because the 1kGP data is used extensively, we find that the batch effects also lead to incorrect imputation by leading imputation servers and \todo{some questionable \sout{suspicious}} GWAS associations. Lower-quality data from the early phases of the 1kGP thus continues to \todo{impact \sout{contaminate}} modern studies in hidden ways. \todo{As technologies and analytical methods improve, it is important that we renew and update these data resources accordingly}\sout{It may be time to retire or upgrade such legacy sequencing data.}
\vspace{5mm}
...
\todo{Given this signature had not been reproduced in another dataset}, we \sout{therefore} decided to follow up on this observation, by using a newly sequenced dataset of Japanese individuals from Nagahama.
\todo{Our initial goal was to explore the population structure that might be responsible for such an intriguing mutational signature.}
...
\todo{\textbf{B} The sequencing technologies used over time. A change in technology coincides with an increase in overall quality of data produced.}}
\label{MapQual}
\end{figure}
...
Here we perform a \todo{genotype conditional association test (GCAT), where} genotypes are now the dependent variable that we attempt to predict using the continuous variable $Q$ as the independent variable \citep{song2015testing}.
...
\todo{Statistical significance of these enrichment is computed using a chi-squared test as described by Harris 2017 \citep{Harris2017a}. }

Thus, this mutational signal is heavily enriched in $Q$-associated SNPs, but residual signal remains in non-significant SNPs, presumably because many rare alleles found in individuals with low $Q$ remain unidentifiable using association techniques \todo{due to lack of power (Supplementary Figure \ref{histogram_of_sig_snps})}.
...
The grey text is the proportion of significant associations per category.
...
This is more than our nominal false discovery rate of \todo{$\alpha = 0.01$} of the sites. 
...
\todo{In the GCAT test, a variant may be significantly associated to $Q$ in some populations, but not necessarily all populations. 
...
There are 5 \todo{variants} that show differing frequencies between both datasets that are likely explained by biased genotypes.
...
\todo{These  $Q$-associated variants could also compromise the imputation of real variants that happen to be  close by, causing not only improper imputation, but also false negatives for real variants.}
...
\todo{Some of these publications noted that these variants were not flanked by linkage disequilibrium peaks \citep{Mandage2017}.
...
\todo{It is unlikely that EBV copy number associations to these seven $Q$-associated variants could be explained by population structure as the data quality of an individual is also correlated with sequencing date.
