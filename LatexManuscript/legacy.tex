...
\todo{Mafessoni et al. recently identified a batch effect in the 1kGP without using any quality control metrics, but instead, using an approach that identifies individuals and loci with unusual linkage disequilibrium patterns, these result partially overlap with the variants we identified using a more statistically powerful method \citep{mafessoni2018turning}.}8
Here we point out how these and additional unreported batch effects in the early phases of the 1kGP still lead to incorrect genetic conclusions through population genetic analyses and spurious GWAS associations as a result of imputation using the 1kGP as a reference.
...
\todo{
To assess whether or not the variants we've identified are the result of cell line or technical artifact, we compare the original 1kGP sequence data to more recent sequencing efforts using the same cell lines.
Variants that do not reproduce across these cohorts are likely the result of technical artifacts, however variants that do reproduce across cohorts are either the result of a cell line artifact or biased genotypes.
 }
...
\todo{This is more than the 2.96 predicted false positives according to our nominal false discovery rate of $\alpha = 0.01$}. 
...
\todo{The higher rate of variants that reproduce across cohorts might be due to the fact that the average effect sizes are smaller in the GCAT model.}
