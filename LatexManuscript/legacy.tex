...
\todo{Mafessoni et al. recently identified a batch effect in the 1kGP without using any quality control metrics, but instead, using an approach that identifies individuals and loci with unusual linkage disequilibrium patterns, these result partially overlap with the variants we identified using a more statistically powerful method \citep{mafessoni2018turning}.}\sgcomment{You are talking about things that have not been introduced yet... Maybe: Mafessoni et al. recently identified batch effects in the 1kGP by looking for individuals with excess linkage disequilibrium among distant variants. 
}
...
\caption{
Suspicious mutations carried by individuals with low quality data have distinct mutational profiles, reproduce poorly across studies \sgcomment{, and are distributed across the genome--this is outdated}. 
\textbf{A} 
...
($587$ SNPs had  $p < 10^{-8}.$)
\sgcomment{Why do we say the following?} The same analysis was performed independently for each of the populations in the 1000 Genomes Project.
\textbf{B} 
...
The shape indicates whether the individual \sgcomment{belongs to -- could we clarify this?} Phase 1 or Phase 3 of the 1000 Genomes project. 
...
\sgcomment{There seems to be a paragraph break needed, a logical link missing, or both?}\todo{The mutational 3-mer substitution patterns of this list of variants is similar to the signature identified in the single population test of the 1kGP JPT in that there is an enrichment in *AC${\rightarrow}$*CC.
