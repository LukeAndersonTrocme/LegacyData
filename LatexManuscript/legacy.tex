...
\todo{A recent publication by Mafessoni et al. also identified a batch effect in the 1kGP using a method that is independent of quality metrics \citep{mafessoni2018turning}.
<<<<<<< Updated upstream
They identify over 15,000 variants significantly associated to abnormal LD patterns that are not associated to $Q$ 
\sgcomment{the previous phrase is confusing to me. E.g., does "that" stand for patterns or variants?
Maybe replace by :  
A recent publication by Mafessoni et al. also identified a batch effect in the 1kGP using a method that uses linkage disequilibrium rather than quality metrics to identify over 15,000 suspicious variants \citep{mafessoni2018turning}.
  } (Figure \ref{Mafessoni_MutSpect}).
...
=======
They identify over 15,000 variants significantly associated to abnormal LD patterns that are not associated to $Q$ (Figure \ref{Mafessoni_MutSpect}).
We find that 3\% of the variants they identified are also in our list of suspicious variants and have correlated p-values to those identified using the GCAT method (Figure \ref{Mafessoni_MutSpect}).
We also find 23,172 $Q$-associated variants that are not associated to abnormal LD patterns. 
Interetingly, the variants identified my Mafessoni et al. are not enriched in the mutational spectra described above (Figure \ref{Mafessoni_MutSpect}).
These results indicate that there may a multitude of batch effects in the 1kGP that can only be identified using a suit of association tests.}
>>>>>>> Stashed changes
...
=======
To assess whether or not $Q$-associated variants result from mutations occurring in the creation or maintenance of the cell lines, or from technical artifacts, we compare the original 1kGP sequence data to more recent sequencing efforts using the same cell lines.
Variants that do not reproduce across these cohorts are likely the result of technical artifacts. 
By contrast, if the same mutations are observed at identical frequencies in independently sequenced cohorts, we can conclude that recurrent cell line mutations are likely to blame. 
Finally, to complicate matters, we may have a third situation where a real variant has a genotyping error probability that correlates with $Q$. 
In this case, we expect the variant may reproduce, but with mismatched genotypes in some individuals.
>>>>>>> Stashed changes
 }
...
Some of these publications noted that these variants were not flanked by LD peaks \citep{Mandage2017}.
...
\todo{It is plausible that the EBV copy number phenotype is sensitive to mapping and confounded by the same technical artefacts that lead to biased SNP calling.} \sgcomment{Don't forget to explain that the authors dealt with this using another QC metric!}
...
\todo{Recommendations : doing their approach as well as ours. We recommend removing the variants we ID and the ones they find. Regression with QC metrics (shoutout to them)}
...
There are a total of 40,266 variants in this plot with only 1328 ( 3\% ) are statistically significant in both test.
There are 23172  variants ( 58\% ) that are $Q$-associated but not LD-associated and 15766 ( 39\% ) that are LD-associated but not $Q$-associated.
