...
ggplot(plt, aes(x=ordered, y= as.numeric(X.ALLELE.FREQ), color=ordered, group=1))+geom_line()+geom_point(shape=15)+scale_color_manual(values=MyColour)+facet_wrap(.~ POS,ncol=1)+theme_classic()+theme(strip.background=element_blank(), strip.text=element_blank(), axis.title=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank(),axis.line=element_blank(), axis.text.x=element_text(angle=90, size=5, vjust=-0.05))+guides(color=F)+geom_point(data=plt[complete.cases(plt),], shape=1, size=4, color='black')
