...
snp<-read.table('~/Documents/QualityPaper/31384880.txt',sep=',')
dev<-read.table('~/Documents/Regression/31384880.dev.txt')
...
ggplot(sig, aes(x=V6, y=V4.x))+geom_point(shape=1)+facet_wrap(~Name,ncol=1)+geom_smooth(method = "glm",     method.args = list(family = "binomial"), se = FALSE,size=0.5)+theme_classic()+labs(x='Quality', y='Genotype')+scale_y_continuous(breaks=c(0,1))
...
snp<-read.table('~/Documents/QualityPaper/33264017.txt',sep=',', fill=T)
dev<-read.table('~/Documents/QualityPaper/33264017.dev.txt')
dev$p<-- pchisq(dev$V4, 1, lower.tail=F, log.p=T)/log(10)
dev<-dev[which(dev$p>6),]
